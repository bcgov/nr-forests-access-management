...
function onError(error: any, info: string) {
    ...
    const genericErrorMsg =
        'An application error has occurred. Please try again. If the error persists contact support.';
    ...
    if (axios.isAxiosError(error)) {
        ...
        const e403_authorizationErrorMsg =
            'You do not have the necessary authorization for the requested action.';
