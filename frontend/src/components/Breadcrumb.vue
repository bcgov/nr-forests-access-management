            ...
            <template v-else>
                <template v-if="applicationsUserAdministers.length > 1">
                    ...
                    <RouterLink to="/application"
                        >Select Application</RouterLink
                    >
                </template>
                <template v-if="$router.currentRoute.value.path == '/manage'">
                    &rarr; Manage Access:
                    {{ selectedApplicationShortDisplayText }}
                </template>
                <template v-else>
                    ...
                    <template
                        v-if="$router.currentRoute.value.path == '/grant'"
                    >
                        &rarr; Grant Access:
                        {{ selectedApplicationShortDisplayText }}
                    </template>
                    <template v-else> &rarr; ERROR SHOULD NOT REACH </template>
                </template>
            </template>
        </template>
    </span>
