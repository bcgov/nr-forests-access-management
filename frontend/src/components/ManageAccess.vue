...
function showingMessage(): string {
    ...
    return (
        ...
        visible +
        ' of ' +
        userRoleAssignments.value.length +
        ' records'
    );
}

function filterIncludes(
    userRoleAssignment: FamApplicationUserRoleAssignmentGet
): boolean {
    if (userFilter.value != null) {
        if (
            !userRoleAssignment.user.user_name
                .toLocaleUpperCase()
                .includes(userFilter.value.toLocaleUpperCase())
        ) {
            return false;
        }
    }
    ...
    if (roleFilter.value != null) {
        if (
            !userRoleAssignment.role.role_name
                .toLocaleUpperCase()
                .includes(roleFilter.value.toLocaleUpperCase())
        ) {
            return false;
        }
    }
    ...
    if (
        forestClientFilter.value != null &&
        forestClientFilter.value.length > 0
    ) {
        if (userRoleAssignment.role.client_number == null) {
            ...
            !(
                String(
                    userRoleAssignment.role.client_number.forest_client_number
                ).includes(forestClientFilter.value)
...
<template>
    <div>
        ...
        <br />
        <template
            v-if="userRoleAssignments != null && userRoleAssignments.length > 0"
        >
            <span><strong>Filter By:</strong></span>
            <span>
                Username
                <input placeholder="username" v-model="userFilter" size="12"
            /></span>
            &nbsp;
            <span
                >Role <input placeholder="role" v-model="roleFilter" size="12"
            /></span>
            &nbsp;
            <span
                >Forest Client
                <input
                    placeholder="forest client"
                    v-model="forestClientFilter"
                    size="8"
            /></span>
            &nbsp;
                ...
                <tbody>
                    <template v-for="assignment in userRoleAssignments">
                        <tr v-if="filterIncludes(assignment)">
                            ...
                            <td v-if="assignment.role.parent_role">
                                {{ assignment.role.parent_role?.role_name }}
                            </td>
                            <td v-else>{{ assignment.role.role_name }}</td>
                            <td v-if="assignment.role.client_number">
                                {{
                                    assignment.role.client_number
        ...
        <template v-else> No user role assignments found. </template>
    </div>
</template>
