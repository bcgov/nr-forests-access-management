...
<template>
    ...
    <div v-if="props.permissionDetails.permission_type === PrivilegeDetailsPermissionTypeEnum.EndUser">
        <div v-for="role in props.permissionDetails.roles" :key="role.role" class="permission-details-col-container">
            <div class="permission-type-container">
                <p>Role: </p>
                <Chip :label="role.role" />
            </div>
            <OrganizationsList v-if="role.scopes?.find(scope => scope !== null)" :scopes="role.scopes" />
        </div>
    </div>
