...
class FamApplication(Base):
    ...
    application_client_id = Column(
        Integer, ForeignKey("fam_application_client.application_client_id")
    )
    create_user = Column(String(30), nullable=False)
    create_date = Column(
        TIMESTAMP(precision=6), nullable=False, server_default=text("CURRENT_DATE")
...
t_fam_application_group_xref = Table(
    ...
    Column(
        "application_id",
        Integer,
        ForeignKey("fam_application.application_id"),
        primary_key=True,
        nullable=False,
    ),
)
...
t_fam_user_group_xref = Table(
    ...
    Column(
        "group_id", ForeignKey("fam_group.group_id"), primary_key=True, nullable=False
    ),
)
