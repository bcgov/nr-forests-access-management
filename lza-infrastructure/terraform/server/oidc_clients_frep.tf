resource "aws_cognito_user_pool_client" "dev_frep_oidc_client" {
  ...
  logout_urls                                   = concat(
      ...
      [for i in range("${var.dev_pr_url_count}") : "https://nr-frep-${i}.apps.gold.devops.gov.bc.ca"]
  )
...
resource "aws_cognito_user_pool_client" "test_frep_oidc_client" {
  ...
  logout_urls                                   = [
    "http://localhost:3000",
    "https://nr-frep-test.apps.gold.devops.gov.bc.ca"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_USER_SRP_AUTH"]
...
resource "aws_cognito_user_pool_client" "prod_frep_oidc_client" {
  ...
  allowed_oauth_scopes                          = ["openid", "profile", "email"]
  callback_urls                                 = [
    "https://frep.nrs.gov.bc.ca/auth/callback"
  ]
  logout_urls                                   = [
    "https://frep.nrs.gov.bc.ca"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_USER_SRP_AUTH"]
