resource "aws_cognito_user_pool_client" "dev_frep_oidc_client" {
  ...
  callback_urls                                 = [
    ...
    "http://localhost:3000",
    "https://frep-dev.apps.silver.devops.gov.bc.ca"
  ]
  logout_urls                                   = [
    var.oidc_sso_playground_url,
    "${var.cognito_app_client_logout_chain_url.dev}http://localhost:3000",
    "${var.cognito_app_client_logout_chain_url.dev}https://frep-dev.apps.silver.devops.gov.bc.ca/logout"
  ]
...
resource "aws_cognito_user_pool_client" "test_frep_oidc_client" {
  ...
  callback_urls                                 = [
    var.oidc_sso_playground_url,
    "http://localhost:3000",
    "https://frep-test.apps.silver.devops.gov.bc.ca"
  ]
  logout_urls                                   = [
    var.oidc_sso_playground_url,
    "${var.cognito_app_client_logout_chain_url.test}http://localhost:3000",
    "${var.cognito_app_client_logout_chain_url.test}https://frep-test.apps.silver.devops.gov.bc.ca/logout"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_REFRESH_TOKEN_AUTH"]
...
resource "aws_cognito_user_pool_client" "prod_frep_oidc_client" {
  ...
  callback_urls                                 = [
    var.oidc_sso_playground_url,
    "https://apps.nrs.gov.bc.ca/frep"
  ]
  logout_urls                                   = [
    var.oidc_sso_playground_url,
    "${var.cognito_app_client_logout_chain_url.prod}https://apps.nrs.gov.bc.ca/frep/logout"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_REFRESH_TOKEN_AUTH"]
