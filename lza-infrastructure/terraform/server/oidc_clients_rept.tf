resource "aws_cognito_user_pool_client" "dev_rept_oidc_client" {
  ...
  logout_urls                                   = concat(
    ...
    [for i in range("${var.dev_pr_url_count}") : "https://nr-rept-${i}.apps.silver.devops.gov.bc.ca"]
  )
...
resource "aws_cognito_user_pool_client" "test_rept_oidc_client" {
  ...
  logout_urls                                   = [
    "https://nr-rept-test.apps.silver.devops.gov.bc.ca"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_USER_SRP_AUTH"]
...
resource "aws_cognito_user_pool_client" "prod_rept_oidc_client" {
  ...
  logout_urls                                   = [
    "https://nr-rept-prod.apps.silver.devops.gov.bc.ca"
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_USER_SRP_AUTH"]
