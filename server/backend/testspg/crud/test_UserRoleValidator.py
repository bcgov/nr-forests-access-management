...
sample_forest_client_return = [
    {
        "clientNumber": ACCESS_GRANT_FOM_DEV_AR_00000001_IDIR["forest_client_number"],
...
def test_init_with_request_param_client_number_too_long_error():
    request_dict = {
        **ACCESS_GRANT_FOM_DEV_AR_00000001_IDIR,
        "forest_client_number": FC_NUMBER_LEN_TOO_LONG,
    }
    with pytest.raises(
        ValidationError, match="String should have at most 8 characters"
    ):
...
def test_forest_client_number_exists(
    ...
    request_dict = {**ACCESS_GRANT_FOM_DEV_AR_00000001_IDIR, "forest_client_number": client_id_to_test}
...
def test_get_forst_client(
    ...
    mock_forest_client.find_by_client_number.assert_called_with(client_id_to_test)
    assert (
        result is mock_forest_client.find_by_client_number.return_value[0]
