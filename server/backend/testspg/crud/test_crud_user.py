...
def test_get_users(db_pg_connection: Session):
    users = crud_user.get_users(db_pg_connection)
    assert len(users) > 1


def test_get_user(db_pg_connection: Session):
    # get non exists user id
    user = crud_user.get_user(db_pg_connection, 0)
    assert user is None

    # get user
    user = crud_user.get_user(db_pg_connection, 1)
    assert user is not None
    assert user.user_id == 1


def test_get_user_by_domain_and_name(db_pg_connection: Session):
...
def test_delete_user(db_pg_connection: Session):
    # create a user
    request_user = schemas.FamUser(**TEST_NEW_USER)
    new_user = crud_user.create_user(fam_user=request_user, db=db_pg_connection)
...
def test_find_or_create(db_pg_connection: Session):
    ...
    initial_users = crud_user.get_users(db_pg_connection)
    # give the new user
    ...
    after_add_users = crud_user.get_users(db_pg_connection)
