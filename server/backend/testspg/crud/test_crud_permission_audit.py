...
# No Matching Records
def test_read_permission_audit_history_no_matching_records(db: Session):
...
# Invalid Data Types
def test_read_permission_audit_history_invalid_data_types(db: Session):
    with pytest.raises(ValueError):
        read_permission_audit_history_by_user_and_application(
            "invalid_user_id", APPLICATION_ID_1, db
        )


# Multiple Users, Same Application
def test_read_permission_audit_history_multiple_users_same_application(db: Session):
    # Add records for two different users but the same application
    db.add(AUDIT_RECORD_1)
    db.add(AUDIT_RECORD_2)
    db.commit()

    result = read_permission_audit_history_by_user_and_application(
        USER_ID_1, APPLICATION_ID_1, db
    )

    # Ensure only USER_ID_1's record is returned
    assert len(result) == 1
    assert result[0].change_target_user_id == USER_ID_1
    assert result[0].change_performer_user_details["username"] == "user1"


# Multiple Applications, Same User
def test_read_permission_audit_history_multiple_applications_same_user(db: Session):
    # Add records for the same user but different applications
    db.add(AUDIT_RECORD_1)
    db.add(AUDIT_RECORD_3)
    db.commit()
...
# Valid Case
def test_read_permission_audit_history_by_user_and_application(db: Session):
