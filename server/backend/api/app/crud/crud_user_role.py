...
def createFamUserRoleAssignment(
    ...
    if (
        ...
        raise RuntimeError(f"Invalid user type: {request.user_type}.")
    ...
    fam_user_role_xref = getUserRolebyUserIdAndRoleId(
        db, fam_user.user_id, request.role_id
    )
    ...
    fam_role = crud_role.getFamRole(db, fam_user_role_xref.role_id)
    if not fam_role:
        raise RuntimeError(f"Role id {fam_user_role_xref.role_id} does not exist.")
    LOGGER.debug(f"Role for user_role assignment: {fam_role.role_id}.")

    child_role: models.FamRole = None
    if request.client_number_id:
        LOGGER.debug(
            f"Forest Client Id {request.client_number_id} present in request to assign a forest client role."
        )
        ...
        if not forest_client:
            LOGGER.debug(
                f"Forest Client with Id {request.client_number_id} does not exist, add a new Forest Client."
            )
            ...
            new_fam_forest_client = db.execute()
            LOGGER.debug(
                f"New Forest Client {new_fam_forest_client.client_number_id} added."
            )

        # Insert forest-client child role if client_number_id present...
    ...
    new_fam_user_role: models.FamUserRoleXref = models.FamUserRoleXref(
        {
            ...
            "role_id": (fam_role.role_id if not child_role else child_role.role_id),
            "create_user": famConstants.FAM_SYSTEM_USER,
        }
    )
