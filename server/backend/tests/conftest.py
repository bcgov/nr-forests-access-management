...
def loadEnvs():
    #'server/backend/tests/env-testing'
    testingDotEnvFilePath = os.path.join(
        os.path.dirname(__file__), 'env-testing')
    dotenv.load_dotenv(testingDotEnvFilePath)
    usePostgres = os.getenv("USE_POSTGRES", "false")
    LOGGER.debug(f"env: {usePostgres}")

@pytest.fixture(scope="function")
def getApp(sessionObjects, loadEnvs, dbEngine: Engine) -> Generator[FastAPI, Any, None]:
    """
...
# This @event is important. By default FOREIGN KEY constraints have no effect on
# the operation of the table from SQLite.
