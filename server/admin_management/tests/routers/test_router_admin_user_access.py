...
def test_get_admin_user_access__user_with_app_admin_and_delegated_admin_privilege(
    ...
    setup_new_fom_delegated_admin(new_user.user_id, RoleType.ROLE_TYPE_CONCRETE,
        AppEnv.APP_ENV_TYPE_DEV)
    # assign IDIR new_user FOM_SUBMITTER delegated admin role to FOM_DEV
    dga_submitter_forest_clients = [TEST_FOREST_CLIENT_NUMBER, TEST_FOREST_CLIENT_NUMBER_TWO]
    setup_new_fom_delegated_admin(new_user.user_id, RoleType.ROLE_TYPE_ABSTRACT,
        AppEnv.APP_ENV_TYPE_DEV, dga_submitter_forest_clients)
    ...
    access_app_admin = list(filter(
        lambda x: x["auth_key"] == AdminRoleAuthGroup.APP_ADMIN, result))[0]
    ...
    access_delegated_admin = list(filter(
        lambda x: x["auth_key"] == AdminRoleAuthGroup.DELEGATED_ADMIN, result))[0]
    ...
    # reviewer role
    fom_dev_reviewer_grant = list(filter(
        lambda x: x["id"] == TEST_FOM_DEV_REVIEWER_ROLE_ID, fom_dev_roles_grants
    ))[0]
    ...
    # submitter role
    fom_dev_submitter_grant = list(filter(
        lambda x: x["id"] == TEST_FOM_DEV_SUBMITTER_ROLE_ID, fom_dev_roles_grants
    ))[0]
    ...
    assert set(fom_dev_submitter_grant["forest_clients"]) == set(dga_submitter_forest_clients)