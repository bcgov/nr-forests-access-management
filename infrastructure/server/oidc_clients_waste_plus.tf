resource "aws_cognito_user_pool_client" "dev_waste_plus_oidc_client" {
...
resource "aws_cognito_user_pool_client" "test_waste_plus_oidc_client" {
  access_token_validity                         = "5"
  allowed_oauth_flows                           = ["code"]
  allowed_oauth_flows_user_pool_client          = "true"
  allowed_oauth_scopes                          = ["openid", "profile", "email"]
...
resource "aws_cognito_user_pool_client" "prod_waste_plus_oidc_client" {
  access_token_validity                         = "5"
  allowed_oauth_flows                           = ["code"]
  allowed_oauth_flows_user_pool_client          = "true"
  allowed_oauth_scopes                          = ["openid", "profile", "email"]
  callback_urls                                 = [
    var.oidc_sso_playground_url,
    "https://nr-waste-plus-prod-frontend.apps.silver.devops.gov.bc.ca/dashboard",
    "https://wasteplus.nrs.gov.bc.ca/dashboard"
  ]
  logout_urls                                   = [
    var.oidc_sso_playground_url,
    "${var.cognito_app_client_logout_chain_url.prod}https://nr-waste-plus-prod-frontend.apps.silver.devops.gov.bc.ca",
    "${var.cognito_app_client_logout_chain_url.prod}https://wasteplus.nrs.gov.bc.ca",
  ]
  enable_propagate_additional_user_context_data = "false"
  enable_token_revocation                       = "true"
  explicit_auth_flows                           = ["ALLOW_REFRESH_TOKEN_AUTH"]
  id_token_validity                             = "60"
  name                                          = "waste_plus_prod"
  prevent_user_existence_errors                 = "ENABLED"
  read_attributes                               = var.minimum_oidc_attribute_list
  refresh_token_validity                        = "24"
  supported_identity_providers                  = [
